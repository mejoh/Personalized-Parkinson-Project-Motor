function nonmri2bids_pit(force, verbose, POMids)
...
% spm12
rawpath  = '/project/3024006.01/raw';
bidspath = '/project/3024006.01/bids';
...
if nargin<3 || isempty(POMids)
    POMDirs = spm_select('List', rawpath, 'dir', '^sub-PIT2.*');
    POMids  = cellstr(POMDirs(:,5:end));
end
bidsdirs = spm_select('List', bidspath, 'dir', '^sub-PIT2.*');
...
general.dataset_description.Name               = 'PIT - ParkinsonInToom';
...
for n = 1:numel(POMids)
    ...
    visit = dir(fullfile(bidspath, ['sub-' POMids{n}], 'ses-PIT*'));
    nVisits = size(visit);
    nVisits = nVisits(1,1);
    for v = 1:nVisits
        if isempty(visit) || ~startsWith(visit(v).name, 'ses-PITVisit')
            warning('Unknown visit found for: %s/%s', POMids{n}, visit(v).name)
            continue
        elseif ~any(strcmp(POMids{n}, bidsids))
            warning('%s/%s not in BIDS directory', POMids{n}, visit(v).name)
            continue
        else
            visit = visit.name(5:end);
        end
        funclist = spm_select('List', fullfile(bidspath, ['sub-' POMids{n}], ['ses-' visit], 'func'));
        if any(contains(cellstr(funclist), 'reward', 'IgnoreCase',true)) && any(contains(cellstr(funclist), 'motor', 'IgnoreCase',true))
            task = cell(2,1);
            task{1} = 'reward';
            task{2} = 'motor';
        elseif any(contains(cellstr(funclist), 'reward', 'IgnoreCase',true))
            task = cellstr('reward');
        elseif any(contains(cellstr(funclist), 'motor', 'IgnoreCase',true))
            task = cellstr('motor');
        else
            warning('Unknown task found for: %s\n%s', POMids{n}, funclist')
            task = 'unknown'; % pause
            % continue
        end
        ...
        for i = 1:numel(task)
            if verbose
                fprintf('\n--> %s (%i/%i): %s\n', POMids{n}, n, numel(POMids), task{i})
            end
    
            %% EMG
            emg2bids(general, POMids{n}, rawpath, bidspath, task{i}, visit, force, verbose)
            ...
            %% SMI Eyetracker data
            eye2bids(general, POMids{n}, rawpath, bidspath, task{i}, visit, force, verbose)
