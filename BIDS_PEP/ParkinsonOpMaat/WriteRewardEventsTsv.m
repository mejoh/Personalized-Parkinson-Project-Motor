...
% %Do POM
newFilePath = fullfile("/project", "3024010.01", "bids");
pomSubsDir = dir(fullfile("/project", "3024010.01", "bids", "sub-MRI*"));
...
rowfun(@(cSub) CreateEventsTSV(cSub, newFilePath), rewardPomSubs, 'NumOutputs', 0);

%Single sub
% cSub = "sub-MRI01071507";
% CreateEventsTSV(cSub, newFilePath)
% ----
% testSub = "sub-POM1FM0416036";
% retrieveFiles(testSub)

% FUNCTION TO CHECK POM TASK FROM RAW
function [cFileName, isReward] = checkPomTask(cSub)

cFileDir = dir(fullfile("/project", "3024010.01", "raw", cSub, "ses-mri*", "beh", "*.log"));
cFileName = string(extractBetween(cFileDir(end).name, "run", "Task"));
...
function [pracTable, taskTable, mriTable, cMeta] = retrieveFiles(cSub, visit)

%Construct file names
cPracFile = spm_select('FPList',fullfile("/project", "3024010.01", "raw", cSub, visit, "beh"), strcat(cSub, ".*reward_prac.*.txt$"));
cTaskFile = spm_select('FPList',fullfile("/project", "3024010.01", "raw", cSub, visit, "beh"), strcat(cSub, ".*reward_mri.*.txt$"));
cMriFile = spm_select('FPList',fullfile("/project", "3024010.01", "raw", cSub, visit, "beh"), strcat(cSub, ".*reward_mri.*.log$"));
...
elseif exist(cTaskFile, 'file') && exist(cMriFile, 'file')
    taskFile = cTaskFile;
    mriFile = cMriFile;
...
%Read Meta
meta = extractBetween(mriFile,"_fmri",".log");
cMeta.Hand = string(extractBefore(meta,"_"));
cMeta.Lang = string(extractAfter(meta,"_"));
...
cMeta.group = string(extractBetween(mriFile, strcat(strrep(visit,"mri",""),"_"), "_reward"));

end

% FUNCTION TO CONVERT RAW TABLES INTO EVENTS.TSV
function CreateEventsTSV(cSub, newFilePath)

visits = dir(fullfile(newFilePath, cSub, 'ses-mri0*'));

for v = 1:numel(visits)

    newSubDir = fullfile(newFilePath,  cSub, strcat(visits(v).name, "/beh"));
    if ~exist(newSubDir, 'dir'); mkdir(newSubDir); end
    newFileName = fullfile(newSubDir, strcat(cSub, "_", visits(v).name, "_task-reward_acq-ME5_run-1_events.tsv"));
    newJsonFileName = strrep(newFileName, ".tsv", ".json");
    trainingFileName = fullfile(newSubDir, strcat(cSub, "_", visits(v).name, "_task-reward_acq-practice_run-1_events.tsv"));

    if ~exist(newFileName, 'file')
        %Load files
        [trainingFile, customFile, standardFile, cMeta] = retrieveFiles(cSub, visits(v).name);
        ...
        eventsTable.onset = round(eventsTable.onset/1000,3);
        eventsTable.duration = round(eventsTable.duration/1000,3);
        ...
        JSON.Group.Levels.HC = "Healthy control";
        JSON.Group.Levels.OFF = "Patient, assessed in an off-medicated state (<12h withdrawal)";
        JSON.Group.Levels.ON = "Patient, assessed in an on-medicated state";
