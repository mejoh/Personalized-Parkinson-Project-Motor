function motor_2ndlevel_PairedTtests(Swap)
...
for c = 1:numel(ConList)
    ...
    for n = 1:numel(unique(SubInfo.Sub))
        Img_off = dir(fullfile(ANALYSESDir, 'Group', ConList{c}, ['PD_PIT_' SubInfo.Sub{n} '*' ConList{c} '*.nii']));
        Img_off = strcat(Img_off.folder, '/', Img_off.name);
        Img_on = dir(fullfile(ANALYSESDir, 'Group', ConList{c}, ['PD_POM_' SubInfo.Sub{n} '*' ConList{c} '*.nii']));
