...
summary <- df.AFC %>%
        group_by(session) %>% 
        summarise(Avg.FWHM_x = mean(FWHM_x),
        ...
        select(-session) %>%
        summarise(Avg.FWHM_x = mean(Avg.FWHM_x),
                  Avg.FWHM_y = mean(Avg.FWHM_y),
                  Avg.FWHM_z = mean(Avg.FWHM_z),
                  Avg.FWHM_comb = mean(Avg.FWHM_comb),
                  Sd.FWHM_x = sd(Sd.FWHM_x),
                  Sd.FWHM_y = sd(Sd.FWHM_y),
                  Sd.FWHM_z = sd(Sd.FWHM_z),
                  Sd.FWHM_comb = sd(Sd.FWHM_comb))
