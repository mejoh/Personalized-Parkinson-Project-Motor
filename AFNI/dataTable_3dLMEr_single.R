dataTable_3dLMEr_single <- function(con='con_0005.nii', outputDir='P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI'){
  ...
  df.all %>%
    ...
    ggplot(aes(y=YearsToFollowUp,x=Subtype)) +
    ylab('Years to follow-up') +
    geom_boxplot()
  df.weeks <- df.all %>%
    filter(TimepointNr=='T1') %>%
    select(Group, Subtype, YearsToFollowUp)
  m.group <- lm(YearsToFollowUp ~ Group, data = df.weeks)
  m.subtype <- lm(YearsToFollowUp ~ Subtype, data = df.weeks)
  ...
  #   select(any_of(vars.disease))
  # outputName.disease.poly2 <- str_replace(outputName, 'dataTable', 'disease-poly2_dataTable')
  # vars.disease <- c('Subj', 'Group', 'Age', 'Age.poly1', 'Age.poly2', 'Age.poly3', 'Sex', 'InputFile')
  # df.disease.poly3 <- df.disease %>%
  #   select(any_of(vars.disease))
  # outputName.disease.poly3 <- str_replace(outputName, 'dataTable', 'disease-poly3_dataTable')
  ...
  # vars.severity <- c('Subj', 'ClinScore', 'ClinScore.poly1', 'ClinScore.poly2', 'ClinScore.poly3', 'Age', 'Sex', 'InputFile')
  # df.severity.poly3 <- df.severity %>%
  #   select(any_of(vars.severity))
  # outputName.severity.poly3 <- str_replace(outputName, 'dataTable', 'severity-poly3_dataTable')
  
  ##### Write to file #####
  write_delim(df.disease.poly1, outputName.disease.poly1, delim = '\t') 
  # write_delim(df.disease.poly2, outputName.disease.poly2, delim = '\t') 
  # write_delim(df.disease.poly3, outputName.disease.poly3, delim = '\t') 
  # write_delim(df.subtype, outputName.subtype, delim = '\t')
  write_delim(df.severity.poly1, outputName.severity.poly1, delim = '\t')
  # write_delim(df.severity.poly2, outputName.severity.poly2, delim = '\t')
  # write_delim(df.severity.poly3, outputName.severity.poly3, delim = '\t')
