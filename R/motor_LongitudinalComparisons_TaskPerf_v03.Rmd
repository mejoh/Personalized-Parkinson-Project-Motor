...
##### Correlate bradykinesia in on- and off-states
tmp <- dfClin %>%
        filter(ParticipantType=='PD_POM',
               TimepointNr != 1) %>%
...
g <- ggemmeans(m, terms='Up3OnBradySum_Td') %>% 
        ...
        annotate(geom = 'text', 
                 ...
                 label=paste0('N=',nrow(tmp),', ', 
                              'β=', signif(s$coefficients[2,1],digits=5),', ',
...
tmp %>% write_csv('P:/3024006.02/Analyses/NatComm/Fig2/PanelA.csv')
...
##### Bradykinesia: Time x Medication effect, controlling for baseline MoCA and LEDD
tmp.ba <- dfClin %>%
        filter(TimepointNr == 0) %>%
        select(pseudonym,Age,Gender,YearsSinceDiag,NpsEducYears,z_MoCA__total, LEDD) %>%
...
tmp <- dfClin %>%
        filter(TimepointNr != 1) %>%
        select(pseudonym,TimepointNr,Up3OfBradySum,Up3OnBradySum) %>%
        pivot_longer(cols = c('Up3OfBradySum','Up3OnBradySum'),
                     names_to = 'Medication',
                     values_to = 'Up3BradySum') %>%
        mutate(Medication=factor(Medication,
                                 ...
                                 labels=c('Off','On')),
               TimepointNr=factor(TimepointNr)) %>%
        left_join(., tmp.ba, by = 'pseudonym') %>%
        na.omit()
n <- tmp %>% select(TimepointNr,Medication) %>% table()
print(n)
m <- lmer(Up3BradySum ~ TimepointNr*Medication + Age + Gender + YearsSinceDiag + NpsEducYears + z_MoCA__total + LEDD + (1+TimepointNr|pseudonym), data = tmp)
tab_model(m, digits=5,show.se = T)
a <- Anova(m, type=3)
print(a)
kable(a, 'pipe', digits=Inf) %>% print()
...
##### Bradykinesia progression adjusted for both cognitive and medication progression
tmp.dJoin <- tmp.dMOCA %>%
    select(pseudonym, starts_with('z_MoCA')) %>%
    left_join(., tmp.dLEDD, by = 'pseudonym')
m.dJoin <- lm(Up3BradySum_Td ~ scale(Up3BradySum_T0,scale=F) + 
                      scale(z_MoCA__total_Td,scale=F) + scale(z_MoCA__total_T0,scale=F) + 
                      scale(LEDD_Td,scale=F) + scale(LEDD_T0,scale=F) + 
                      scale(Age,scale=F) + Gender + scale(YearsSinceDiag,scale=F) + scale(NpsEducYears,scale=F), 
              data = tmp.dJoin)
Anova(m.dJoin,type=3) %>% print()
eff <- eta_squared(m.dJoin)
print(eff)
s <- summary(m.dJoin)
tab_model(m.dJoin, digits = 5, show.se = T)
print(s)
...
tmp %>% write_csv('P:/3024006.02/Analyses/NatComm/Fig2/PanelB.csv')
...
dfFMRI.c %>% 
        select(pseudonym,AS.Mean.Delta,Up3BradySum.imp_RawChange,Up3BradySum.imp_T0,z_MoCA__total.imp_RawChange,z_MoCA__total.imp_T0,LEDD.imp_RawChange,LEDD.imp_T0,AS.Mean.T0,Age,Gender,YearsSinceDiag,NpsEducYears,RespHandIsDominant) %>% 
        write_csv('P:/3024006.02/Analyses/NatComm/Fig2/PanelD.csv')
...
# Mean, baseline
dfFMRI.c %>% 
        select(pseudonym,AS.Mean.T0,Up3BradySum.imp_T0,z_MoCA__total.imp_T0,LEDD.imp_T0,Age,Gender,YearsSinceDiag,NpsEducYears,RespHandIsDominant) %>% 
        write_csv('P:/3024006.02/Analyses/NatComm/Fig2/PanelC_01.csv')
...
#Mean, follow-up
dfFMRI.c %>% 
        select(pseudonym,AS.Mean.T2,Up3BradySum.imp_T2,z_MoCA__total.imp_T2,LEDD.imp_T2,Age,Gender,YearsSinceDiag,NpsEducYears,RespHandIsDominant) %>% 
        write_csv('P:/3024006.02/Analyses/NatComm/Fig2/PanelC_02.csv')
...
dat %>%
        select(pseudonym,cid2_delta,Up3BradySum.imp_RawChange,Up3BradySum.imp_T0,cid2_ba,Age,Gender,NpsEducYears,YearsSinceDiag,RespHandIsDominant) %>%
        write_csv('P:/3024006.02/Analyses/NatComm/Fig4/PanelA.csv')
...
df.sphere %>%
        select(SelectionActivity,Up3BradySum.imp,z_MoCA__total.imp,LEDD.imp,Age,Sex,NpsEducYears,RespHandIsDominant) %>%
        write_csv('P:/3024006.02/Analyses/NatComm/Fig3/PanelB.csv')
