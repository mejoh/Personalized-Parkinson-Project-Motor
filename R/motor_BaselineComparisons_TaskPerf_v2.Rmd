...
source("H:/sysneu/marjoh/scripts/RainCloudPlots/tutorial_R/R_rainclouds.R")
source("H:/sysneu/marjoh/scripts/RainCloudPlots/tutorial_R/summarySE.R")
...
```{r datafile}
datafile_task <- 'P:/3022026.01/pep/bids/derivatives/database_motor_task_2021-08-05.csv'
datafile_task <- 'P:/3022026.01/pep/bids/derivatives/database_motor_task_verylongformat_2021-08-30.csv'
datafile_clin_pom <- 'P:/3022026.01/pep/deprecated_ClinVars/derivatives/database_clinical_variables_2021-05-27.csv'
datafile_clin_pit <- 'P:/3022026.01/pep/deprecated_ClinVars/derivatives/database_PIT_clinical_variables_2021-05-21.csv'
datafile_clin_subtypes <- 'P:/3022026.01/pep/ClinVars4/derivatives/Subtypes_01-12-2021.csv'
```
...
data_clin_subtypes <- read_csv(datafile_clin_subtypes) %>% mutate(Subtype = Subtype_DisDurSplit)
...
data_SUBSvsHC <- rbind(data_tmp_hcpit, data_tmp_pdpom) %>%
        mutate(Group = factor(Group, levels = c('HC_PIT','1_Mild-Motor','2_Intermediate','3_Diffuse-Malignant')),
               Condition = as.factor(Condition),
         Block = as.factor(Block))
...
```{r echo=FALSE, message=FALSE}
tmp <- data_SUBSvsHC_RT %>%
        filter(Group == '1_Mild-Motor' | Group == '2_Intermediate') %>%
        mutate(Group = factor(Group))
tmp$Group <- factor(tmp$Group, levels = c('2_Intermediate','1_Mild-Motor'))
...
tmp <- data_SUBSvsHC_RT %>%
        ...
        mutate(Group = factor(Group))
tmp$Group <- factor(tmp$Group, levels = c('3_Diffuse-Malignant','1_Mild-Motor'))
...
tmp <- data_SUBSvsHC_RT %>%
        ...
        mutate(Group = factor(Group))
tmp$Group <- factor(tmp$Group, levels = c('3_Diffuse-Malignant','2_Intermediate'))
