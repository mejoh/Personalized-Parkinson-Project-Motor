classify_subtypes <- function(df, MI=TRUE){
        ...
        df1 <- df1 %>%
                select(pseudonym, TimepointNr, MonthSinceDiag, Updrs2Sum, Updrs3Sum, PIGDavg, MotorComposite,
                       RBDSQSum, SCOPA_AUTSum, CognitiveComposite, MoCASum) %>%
                mutate(TimepointNr = as.factor(TimepointNr))
        #####
        
        ##### Imputation of missing values through stochastic regression #####
        if(MI){
                md.pattern(df1)
                imp <- mice(df1, method = 'norm.nob', m = 1, maxit = 1)
                df1 <- complete(imp) %>% 
                        as_tibble %>%
                        plyr::mutate(MOCA_MCI = if_else(MoCASum >= 26, 0, 1)) %>%
                        select(-c(MoCASum))
                #md.pattern(df1)
        }
        #####
        
        ##### Derive Z-scores for whole cohort and for cohort split at disease duration #####
        CutOff <- median(df1$MonthSinceDiag,na.rm = TRUE)
        df1 <- df1 %>% mutate(DisDurSplit = if_else(MonthSinceDiag >= CutOff,'Above','Below'))
        ...
        df_split <- classification(df_split) %>%
                ...
                select(pseudonym, TimepointNr, Subtype_DisDurSplit, Subtype_DisDurSplit.MCI)
        ...
        df_nosplit <- classification(df_nosplit) %>%
                ...
                select(pseudonym, TimepointNr, MotorComposite, CognitiveComposite, Subtype_NoSplit, Subtype_NoSplit.MCI)
        ...
        df1$Subtype_DisDurSplit.MCI[is.na(df1$Subtype_DisDurSplit.MCI)] <- 'Undefined'
        
        df <- left_join(df, df1, by = 'pseudonym') %>%
                mutate(SubtypeMatches = Subtype_DisDurSplit == Subtype_NoSplit,
                       SubtypeMatches.MCI = Subtype_DisDurSplit.MCI == Subtype_NoSplit.MCI)
        #####
