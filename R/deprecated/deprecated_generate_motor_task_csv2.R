...
generate_motor_task_csv <- function(bidsdir){
                ...
                for(t in Visits){
                        ...
                        for(i in 1:length(Conditions)){
                                ...
                                if(!is.na(Events[[1]]) && !is.na(Events[[2]])){
                                        ...
                                        if(nrow(Row)<1){
                                                msg <- paste(dSubDir, '/', t, ' Cond:', Conditions[i], ' has no usable trials Check sourcefile!', sep = '')
                                                print(msg)
                                                next
                                        }
                                        ...
                                        if(Conditions[i] != 'Catch'){
                                                for(trial in 1:nrow(Row)){
                                                        if(Row[trial,]$correct_response == 'Hit'){
                                                Data <- add_row(Data,
                                                                pseudonym = Subjects[n],
                                                                Timepoint = t,
                                                                Condition = Conditions[i],
                                                                Response.Time = filter(Row[trial,]) %>% pull(var = response_time),
                                                                TrialNumber = filter(Row[trial,]) %>% pull(var = trial_number),
                                                                Block = filter(Row[trial,]) %>% pull(var = block),
        ...
        POM_subs <- Data %>% filter(Group == 'PD_POM' & Condition == 'Ext' & Timepoint == 'ses-POMVisit1') %>% select(pseudonym) %>% unique()
        PIT_subs <- Data %>% filter(Group == 'PD_PIT' & Condition == 'Ext' & Timepoint == 'ses-PITVisit1') %>% select(pseudonym) %>% unique()
        ...
        outputfile <- paste(bidsdir, 'derivatives/database_motor_task_verylongformat_', today(), '.csv', sep = '')
