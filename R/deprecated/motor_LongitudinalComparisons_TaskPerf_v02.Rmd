...
interrogate_cluster <- function(df, cluster, outputpth){
    ...
    g <- df.collapsed %>%
        ...
        ggtitle(t) + 
        geom_text(x=0,y=2.5, label = paste('Bradykinesia*2c>1c: β = ', round(s$coefficients[7,1],digits=5),
                                           ', SE = ', round(s$coefficients[7,2],digits=5),
                                           ', P = ', round(s$coefficients[7,5],digits=10),
                                           sep='')) +
        geom_text(x=0,y=2, label = paste('Bradykinesia*3c>1c: β = ', round(s$coefficients[8,1],digits=5),
                                           ', SE = ', round(s$coefficients[8,2],digits=5),
...
compare_rts <- function(f, tmp, tmp.collapsed){
    ...
    # contrast(em, interaction = c('revpairwise','revpairwise'), adjust = 'mvt') %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
    ...
    # contrast(em, interaction = c('revpairwise'), adjust = 'mvt') %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
    # g <- tmp.collapsed %>%
    ...
    # contrast(em, interaction = c('revpairwise','revpairwise'), adjust='mvt', type='response') %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
    # contrast(em, interaction = c('revpairwise','revpairwise'), adjust='mvt', type='response', by=NULL) %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
    # g <- tmp.collapsed %>%
    ...
    # print(g)
        ...
        contrast(em, interaction = c('revpairwise','revpairwise','revpairwise'),
                ...
                kable(., 'pipe', digits = 5, padding=0) %>% print()
        contrast(em, interaction = c('revpairwise','revpairwise','revpairwise'),
        ...
        kable(em$emmeans,'pipe', digits = 5) %>% print()
        kable(em$contrasts,'pipe', digits = 5) %>% print()
    ...
    #         mutate(ci = se*1.96)
    # kable(emm.summary,'pipe', digits = 5) %>% print()
...
tmp.n <- tmp.collapsed %>%
    ...
    ungroup() %>%
    select(ParticipantType,TimepointNr)
tmp.n %>% select(ParticipantType,TimepointNr) %>% table() %>% print()
...
em <- emmeans(fit, ~ trial_type*TimepointNr|ParticipantType, type='response')
contrast(em, interaction = c('revpairwise','revpairwise','revpairwise'), adjust='mvt', type='response', by=NULL) %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
contrast(em, interaction = c('revpairwise','revpairwise','revpairwise'), adjust='mvt', type='response') %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
...
# Mean
m1 <- lm(AS.Mean.Delta ~ Up3BradySum.imp_RawChange + Up3BradySum.imp_T0 + z_MoCA__total.imp_RawChange + z_MoCA__total.imp_T0 + LEDD.imp_RawChange + LEDD.imp_T0 + AS.Mean.T0 + Age + Gender + YearsSinceDiag + NpsEducYears + RespHandIsDominant, data = dfFMRI.c)
...
# Multiple > Single
m4 <- lm(AS.23gt1.Delta ~ Up3BradySum.imp_RawChange + Up3BradySum.imp_T0 + z_MoCA__total.imp_RawChange + z_MoCA__total.imp_T0 + LEDD.imp_RawChange + LEDD.imp_T0 + AS.23gt1.T0 + Age + Gender + YearsSinceDiag + NpsEducYears + RespHandIsDominant, data = dfFMRI.c)
...
# Mean, baseline
m5 <- lm(log(AS.Mean.T0) ~ Up3BradySum.imp_T0 + z_MoCA__total.imp_T0 + LEDD.imp_T0 + Age + Gender + YearsSinceDiag + NpsEducYears + RespHandIsDominant, data = dfFMRI.c)
...
g <- ggemmeans(m5, terms = 'Up3BradySum.imp_T0') %>% plot(add.data=TRUE,color='darkred') + theme_bw() + ggtitle('Baseline') +
        ...
        annotate(geom='text',label=expression(italic('N')*'=341, '*italic('β')*'=4.4e-03, SE=1.2e-03, '*italic('P')*'=0.001'),x=0,y=0.3, hjust=0) + ylim(0.3,1.6)
print(g)
save_plot('M:/Visualization/R_LongitudinalComparisonFmri/ResponseTimes/RT_BA-mean.png',
                  g, dpi=300, device='png', base_width = 5)
#Mean, follow-up
m6 <- lm(log(AS.Mean.T2) ~ Up3BradySum.imp_T2 + z_MoCA__total.imp_T2 + LEDD.imp_T2 + Age + Gender + YearsSinceDiag + NpsEducYears + RespHandIsDominant, data = dfFMRI.c)
...
g <- ggemmeans(m6, terms = 'Up3BradySum.imp_T2') %>% plot(add.data=TRUE,color='darkred') + theme_bw() + ggtitle('Two-year follow-up') +
        xlab('Bradykinesia severity (ON)') + ylab('Mean response time (s)') + 
        annotate(geom='text',label=expression(italic('N')*'=292, '*italic('β')*'=6.8e-03, SE=1.6e-03, '*italic('P')*'<0.001'),x=0,y=0.3, hjust=0) + ylim(0.3,1.6)
print(g)
...
compare_correct <- function(f, tmp, tmp.collapsed){
    ...
    exp(tab) %>% kable(., 'pipe', digits = 5, padding=0) %>% print()
    ...
    em <- emmeans(fit, revpairwise ~ ParticipantType, type='response', adjust='mvt')
    kable(em$emmeans,'pipe', digits = 5, padding=0) %>% print()
    kable(em$contrasts,'pipe', digits = 5, padding=0) %>% print()
    ggemmeans(fit, terms=c('ParticipantType'), back.transform = TRUE) %>% 
            plot(connect.lines=TRUE)
...
# Mean, bsaline
m5 <- lm(AS.Mean.T0 ~ Up3BradySum.imp_T0 + z_MoCA__total.imp_T0 + LEDD.imp_T0 + Age + Gender + YearsSinceDiag, data = dfFMRI.c)
tab_model(m5, digits=5, show.se=TRUE)
Anova(m5,type=3) %>% print()
...
# Mean, follow-up
m6 <- lm(AS.Mean.T2 ~ Up3BradySum.imp_T2 + z_MoCA__total.imp_T2 + LEDD.imp_T2 + Age + Gender + YearsSinceDiag, data = dfFMRI.c)
tab_model(m6, digits=5, show.se=TRUE)
Anova(m6,type=3) %>% print()
...
# Selection, baseline
m7 <- lm(AS.23gt1.T0 ~ Up3BradySum.imp_T0 + z_MoCA__total.imp_T0 + LEDD.imp_T0 + Age + Gender + YearsSinceDiag, data = dfFMRI.c)
...
# Selection, follow-up
m8 <- lm(AS.23gt1.T2 ~ Up3BradySum.imp_T2 + z_MoCA__total.imp_T2 + LEDD.imp_T2 + Age + Gender + YearsSinceDiag, data = dfFMRI.c)
...
contrast(em, interaction=c('revpairwise','revpairwise'), adjust = 'mvt', by = 'ParticipantType')
contrast(em, interaction=c('revpairwise','revpairwise'), adjust = 'mvt', by = 'TimepointNr')
...
g <- plot(pred, add.data = TRUE,color='darkred') + theme_bw() + ggtitle('Premotor cortex (PMd)') +
...
# Write for glmnet analysis
dat %>%
        select(pseudonym, cid6_ba, cid6_delta) %>%
        mutate(cid6_fu = cid6_ba + cid6_delta) %>%
        rename(premotor_activity_T0 = cid6_ba, premotor_activity_T2 = cid6_fu, premotor_activity_Td = cid6_delta) %>%
...
g1 <- df.sphere %>% 
        ...
        annotate(geom = 'text', x=0, y=-1.9, hjust=0, 
                 label = expression(italic('N')*'=330, '*italic('β')*'=-0.025, SE=0.009, '*italic('P')*'=0.008'),
                 size=3.5)
g2 <- df.sphere %>% 
        filter(TimepointNr==2,
               ROI=='ClustROI') %>%
        ...
        annotate(geom = 'text', x=0, y=-1.9, hjust=0, 
                 label = expression(italic('N')*'=276, '*italic('β')*'=-0.028, SE=0.013, '*italic('P')*'=0.031'),
                 size=3.5)
g <- ggarrange(g1,g2)
print(g)
...
AnalyzeChangeCorrs <- function(dat, cname){
        ...
        tab_model(m,title='Brain-Clinical Associations',digits=5,show.se=T)
        s <- summary(m)
        label <- paste('N=251, β=',round(s$coefficients[9,1],digits = 5), ', SE=', round(s$coefficients[9,2],digits = 5),
                       ', P=', round(s$coefficients[9,5],digits = 5), sep='')
        Anova(m,type=3) %>% print()
        eta_squared(m) %>% print()
        print(s)
        plot(m)
...
label <- paste('N=250, β=',round(s$coefficients[9,1],digits = 5), ', SE=', round(s$coefficients[9,2],digits = 5),
               ', P=', round(s$coefficients[9,5],digits = 5), sep='')
pred <- ggemmeans(m, terms=c('ClinScore_imp_Delta','Region'))
g <- plot(pred, add.data = TRUE) + theme_bw() + ggtitle('') +
...
## Writing data file for elastic net regression (glmnet)

```{r glmnet-data, echo=FALSE, warning=TRUE, message=TRUE, eval=TRUE}

outputfile <- '/project/3024006.02/Data/GLMNET_NatComm/glmnet_func.csv'
