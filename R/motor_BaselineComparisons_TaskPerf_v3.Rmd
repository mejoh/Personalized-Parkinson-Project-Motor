...
clinvars <- c('pseudonym', 'TimepointNr', 'ParticipantType', 'CrossStudyParticipation', 'Subtype_DiagEx3_DisDurSplit', 'Subtype_Imputed_DiagEx3_DisDurSplit',
...
compare_cov <- function(f, tmp, outlier_method = 1){
        ...
        raincloudplot_1Factor(data=tmp, y='Button.Press.CoV', groupvar='ParticipantType',
                              ...
                              xticklabs = c('Control','PD-On'), provided_summary = 'None') %>% print()
...
```{r echo=FALSE, message=FALSE, warning=TRUE}
f <- 'log(Button.Press.CoV) ~ 1 + Up3OfBradySum_Dmean + Age_Dmean + Gender + NpsEducYears_Dmean'
# removed due to non-significance
...
```{r echo=FALSE, message=FALSE, warning=TRUE}
compare_cov_for_medication <- function(f, tmp, outlier_method = 1){
        # Fit model
        print(paste('Fitting the following model:', f, sep=' '))
...
```{r echo=FALSE, message=FALSE, warning=TRUE}
compare_cov_for_subtypes <- function(f, tmp, outlier_method = 1){
        # Fit model
        print(paste('Fitting the following model:', f, sep=' '))
        fitOutliersRetained <- lm(f, data=tmp)
        if(outlier_method == 1){
...
```{r echo=FALSE, message=FALSE, warning=TRUE}
compare_switch <- function(f, tmp, outlier_method = 1){
        # Fit model
        print(paste('Fitting the following model:', f, sep=' '))
        fitOutliersRetained <- lm(f, data=tmp)
        if(outlier_method == 1){
...
```{r echo=FALSE, message=FALSE, warning=TRUE}
compare_switch_for_subtypes <- function(f, tmp, outlier_method = 1){
        # Fit model
        print(paste('Fitting the following model:', f, sep=' '))
        fitOutliersRetained <- lm(f, data=tmp)
        if(outlier_method == 1){
