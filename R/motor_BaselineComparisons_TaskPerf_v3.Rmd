...
```{r poor_performance, echo=FALSE, eval=FALSE}
...
plot <- ggplot(tmp, aes(y=Button.Press.CoV, x=Up3OfBradySum)) +
        ...
        ggtitle('') + 
        xlab('Bradykinesia (MDS-UPDRS III)') +
...
plot <- plot_model(fit, vline.color = 'black',
                   ...
                   axis.labels = c('Cognitive function, Three > One',
                                   'Cognitive function, Two > One',
                                   'Cognitive function'),
                   show.values = T,
                   value.offset = .3,
                   width = .2) + 
...
compare_misses <- function(f, tmp, tmp.collapsed){
        ...
        d = 6
        segmap <- data.frame(
                x =    c(1.00, 1.00,  2.00,  0.80,  1.80,  0.80,  1.20,  1.80,  2.20,  0.80),
                xend = c(2.00, 1.00,  2.00,  1.20,  2.20,  0.80,  1.20,  1.80,  2.20,  1.20),
...
compare_correct <- function(f, tmp, tmp.collapsed){
        ...
        d = 6
        segmap <- data.frame(
                x =    c(1.00, 1.00,  2.00,  0.80,  1.80,  0.80,  1.20,  1.80,  2.20,  1.80,  1.80,  2.00),
                xend = c(2.00, 1.00,  2.00,  1.20,  2.20,  0.80,  1.20,  1.80,  2.20,  2.00,  2.20,  2.20),
...
plot <- ggplot(plotdat, aes(y=error_rate*100, x=Up3OfBradySum, color=trial_type)) +
        ...
        theme(legend.position = c(0.8,1.10),
              legend.key.size = unit(0.5,'cm')) +
        geom_text(x=10,y=55, label = paste('N = ', N, 
                                             ', OR = ', round(exp(tab[4,1]),digits=3),
                                             ', SE = ', round(exp(tab[4,2]),digits=3),
                                             ', P < 0.001', sep=''),
...
compare_correct_for_subtypes <- function(f, tmp, tmp.collapsed){
        ...
        d = 6
        segmap <- data.frame(
                x =    c(1.00, 1.00,  3.00, 0.80,  2.80,  0.80,  1.00,  1.80,  2.00,  2.80,  3.00),
                xend = c(3.00, 1.00,  3.00, 1.20,  3.20,  1.20,  1.20,  2.20,  2.20,  3.20,  3.20),
...
d = 0.6
segmap <- data.frame(
        x =    c(0.90, 0.90,  1.90,  0.80,  1.80,  0.80,  1.00, 1.80,  2.00),
        xend = c(1.90, 0.90,  1.90,  1.00,  2.00,  0.80,  1.00, 1.80,  2.00),
        y =    c(y,    y,     y,     y-d*1, y-d*1, y-d*1, y-d*1, y-d*1, y-d*1),
        yend = c(y,    y-d*1, y-d*1, y-d*1, y-d*1, y-d*2, y-d*2, y-d*2, y-d*2)
)
sigmap <- data.frame(
...
for(v in 1:length(vois)){
        ...
        p <- ggplot(fmridat, aes(x=CognitiveComposite, y=Beta)) + 
                ...
                ggtitle(basename(paste(csvfile, unique(fmridat$Cond), ':', regions[v], 'r=', round(c$estimate, digits = 3), sep = ' '))) + 
                theme_cowplot(font_size = 20) +
                xlab('Cognitive impairment (composite measure)')
        print(p)
...
while(counter <= length(cons)){
        ...
        g <- ggarrange(g.brady,g.cogcom,nrow=2,ncol=1)
        print(g)
