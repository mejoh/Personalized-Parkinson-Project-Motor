...
### Patients who were misdiagnosed at baseline or two-year follow-up
...
cat('Healthy controls versus PD-On')
...
# Post hoc test
emt <- emtrends(fit.clincorr, ~trial_type, var='Up3OfBradySum_Dmean')
contrast(emt, 'revpairwise', adjust = 'mvt') %>% kable(., 'pipe', digits = 3) %>% print()
...
regions <- c('Pre-central (4a)', 'Putamen (MAS)', 'CB (A5)', 'Putamen (LAS)')
...
##### DEPRECATED! Parietal interaction #####
# parietal <- vois[7]
# csvfile <- parietal
# fmridat <- read_csv(csvfile, show_col_types = FALSE) %>%
#         select(-scans) %>%
...
plot1 <- raincloudplot_2Factor(fmridat, 'Beta', c('Cond','Group'),
                              title = 'Inferior parietal lobule\n',
                      xlab='', ylab='Beta',
                      xticklabs = c(paste('HC', ' (n=', N[1],')', sep=''),
                                    paste('MMP', ' (n=', N[2],')', sep='')),
                      legend_title = '',
...
plot <- ggplot(fmridat, aes(x=Up3OfBradySum, y=Beta, color=factor(Region))) + 
        geom_point(alpha = .3, shape = 19, size = 1.5) + 
...
plot <- ggplot(fmridat, aes(x=Up3OfBradySum, y=Beta)) + 
...
N <- fmridat.SPL_putamen %>%
        ...
        unique() %>%
        summarise(N=n())
plot <- ggplot(fmridat.SPL_putamen, aes(x=Up3OfBradySum, y=Beta, color=fct_rev(Region))) + 
...
plot <- ggplot(fmridat.SPL_putamen, aes(x=Up3OfBradySum, y=Beta)) + 
        geom_point(aes(color=Subtype), alpha = .5, shape = 19, size = 1.5) + 
