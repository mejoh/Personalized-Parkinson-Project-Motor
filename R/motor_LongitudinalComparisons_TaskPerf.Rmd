...
library(rmcorr); library(ppcor); library(PResiduals)
...
tmp.collapsed <- dfFMRI %>% 
    group_by(pseudonym, ParticipantType, TimepointNr, trial_type, Up3BradySum) %>%
...
dat <- read_csv('P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI/ON_ANALYSES/ROI/BG_Parietal/3dLME_disease/stats/con_combined_dataTable_21-Apr-2023.txt')
...
# 3dttest++: 3 > 1
# Data reading
dat <- read_csv('P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI/ON_ANALYSES/ROI/BG_Parietal/3dttest++_severity/stats/con_0013_dataTable_09-May-2023.txt')
dat <- dat %>%
        mutate(Subj=str_c('sub-POMU', str_sub(Subj,end=16)),
               Sex=factor(Sex)) %>%
        select(-c(InputFile, voxelwiseBA)) %>%
        rename(C1_BA = con_0013_Severity2_T_Delta_ba_cid1,
               C1_delta = con_0013_Severity2_T_Delta_cid1) %>%
        relocate(Subj, Sex, Age, YearSinceDiag_imp, ClinScore_delta, ClinScore_BA, C1_delta, C1_BA)
...
pred <- ggpredict(m, terms='ClinScore_delta')
g <- plot(pred, add.data = TRUE) + theme_bw() + ggtitle('') +
        ylab('Change in activity (Three > One, Z)') + xlab('Change in bradykinesia (ON)') +
        annotate('text',y=3.5,x=-25,label='N=250, β=-0.047, SE=0.009, P<0.001', hjust=0) +
        ylim(-3.5,3.5)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dttest++_severity_3gt1.png',
                  g, dpi=300, device='png', base_width = 5)
