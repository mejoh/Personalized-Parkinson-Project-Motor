...
##### Data preparation #####
# Files are generated by 'motor_LongitudinalClinConfs.R'
dat.task <- read_csv('P:/3024006.02/Data/matlab/fmri-confs-task_ses-longitudinal_groups-pd_2023-01-23.csv') %>%
        filter(Misdiagnosis == 0)
dat.task.wide <- dat.task %>% 
        pivot_wider(id_cols = c('pseudonym','TimepointNr','ParticipantType','Age','Gender'),
                    names_from = trial_type,
                    names_prefix = 'TT_',
                    values_from = response_time) %>%
        mutate(AS.3gt1 = TT_3choice-TT_1choice,
               AS.2gt1 = TT_2choice-TT_1choice,
               AS.23gt1 = round(((TT_2choice+TT_3choice)/2)-TT_1choice),
               AS.Mean = round((TT_3choice + TT_2choice + TT_1choice)/3)) %>%
        pivot_wider(id_cols = c('pseudonym','ParticipantType','Age','Gender'),
                    names_from = TimepointNr,
                    names_sep = '.',
        ...
        select(pseudonym, ParticipantType, Age, Gender, 
               AS.3gt1.Delta, AS.3gt1.BA, 
               AS.2gt1.Delta, AS.2gt1.BA,
               AS.23gt1.Delta, AS.23gt1.BA,
               AS.Mean.Delta, AS.Mean.BA)

dat.clin <- read_csv('P:/3024006.02/Data/matlab/fmri-confs-clin_ses-longitudinal_groups-pd_2023-01-23.csv') %>%
        filter(Misdiagnosis == 0,
               TimepointNr=='T0' | TimepointNr=='T2')
dat.clin.wide <- pivot_wider(dat.clin,
                        id_cols = c('pseudonym','ParticipantType','Age','Gender'),
                        names_from = TimepointNr,
                        values_from = ClinScore) %>%
        mutate(ClinScore.Delta = T2-T0,
               ClinScore.BA = T0,
               ClinScore.FU = T2) %>% 
        select(pseudonym, ClinScore.Delta, ClinScore.BA, ClinScore.FU)

dat <- left_join(dat.task.wide, dat.clin.wide, by = c('pseudonym')) %>%
        na.omit()

##### Visualization #####

dat %>% select(ClinScore.Delta, AS.Mean.Delta, AS.2gt1.Delta, AS.3gt1.Delta) %>% pairs()

##### Analysis #####

dat.analysis <- dat %>% 
        mutate(AS.Mean.Delta-mean(AS.Mean.Delta),
               AS.2gt1.Delta-mean(AS.2gt1.Delta),
               AS.3gt1.Delta-mean(AS.2gt1.Delta),
               AS.Mean.BA-mean(AS.Mean.BA),
               AS.2gt1.BA-mean(AS.2gt1.BA),
               AS.3gt1.BA-mean(AS.2gt1.BA),
               ClinScore.BA-mean(ClinScore.BA),
               ClinScore.BA-mean(ClinScore.BA),
               ClinScore.BA-mean(ClinScore.BA),
               Gender = factor(Gender),
               Age = Age-mean(Age))

m1 <- lm(ClinScore.Delta ~ AS.Mean.Delta + ClinScore.BA + AS.Mean.BA + Age + Gender, data = dat.analysis)
Anova(m1) %>% print()
g <- ggemmeans(m1, terms = 'AS.Mean.Delta') %>% plot() + theme_bw() + ggtitle('Clinical progression ~ Change in mean response time')
print(g)

m2 <- lm(ClinScore.Delta ~ AS.2gt1.Delta + ClinScore.BA + AS.Mean.BA + Age + Gender, data = dat.analysis)
Anova(m2) %>% print()
g <- ggemmeans(m2, terms = 'AS.2gt1.Delta') %>% plot() + theme_bw() + ggtitle('Clinical progression ~ Change in action-selection (2-1)')
print(g)

m3 <- lm(ClinScore.Delta ~ AS.3gt1.Delta + ClinScore.BA + AS.Mean.BA + Age + Gender, data = dat.analysis)
Anova(m3) %>% print()
g <- ggemmeans(m3, terms = 'AS.3gt1.Delta') %>% plot() + theme_bw() + ggtitle('Clinical progression ~ Change in action selection (3-1)')
print(g)

m4 <- lm(ClinScore.Delta ~ AS.23gt1.Delta + ClinScore.BA + AS.Mean.BA + Age + Gender, data = dat.analysis)
Anova(m4) %>% print()
g <- ggemmeans(m4, terms = 'AS.23gt1.Delta') %>% plot() + theme_bw() + ggtitle('Clinical progression ~ Change in action selection (23-1)')
print(g)
...
```{r roi-beta-disease, echo=FALSE, warning=FALSE, message=FALSE}

betas1 <- read_csv('P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI/ROI_BetaExtraction/con_combined_disease-poly1_23-Jan-2023_dataTable.txt')
...
m <- lmer(Beta ~ Group*TimepointNr*trial_type*Network+(1+TimepointNr+Network|Subj), data = b2)
tab_model(m)
ggemmeans(m, terms=c('TimepointNr','Group','trial_type','Network')) %>% plot()
...
betas1 <- read_csv('P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI/ROI_BetaExtraction/con_combined_severity-poly1_23-Jan-2023_dataTable.txt')

# Define measures of interest (i.e. average betas across pathology and compensation networks)
b1 <- betas1 %>%
        select(Subj, TimepointNr, trial_type, ClinScore_imp2, Age, Sex, starts_with('HCgtPD_Mean'), starts_with('Neg_')) %>%
        select(!contains('PCA')) %>%
        rowwise() %>%
        mutate(Pathology = mean(c_across(contains('HCgtPD_Mean'))),
               Compensation = mean(c_across(contains('Neg_')))) %>%
        select(Subj, TimepointNr, trial_type, ClinScore_imp2, Age, Sex, Pathology, Compensation) 

# Collapse by condition
b2 <- b1 %>%
        pivot_wider(id_cols = c('Subj','TimepointNr','Age','Sex','ClinScore_imp2'),
                    names_from = trial_type,
                    values_from = c(Pathology,Compensation)) %>%
        mutate(Pathology_3gt1 = Pathology_3c - Pathology_1c,
               Pathology_2gt1 = Pathology_2c - Pathology_1c,
               Pathology_Mean = (Pathology_3c + Pathology_2c + Pathology_1c)/3,
               Compensation_3gt1 = Compensation_3c - Compensation_1c,
               Compensation_2gt1 = Compensation_2c - Compensation_1c,
               Compensation_Mean = (Compensation_3c + Compensation_2c + Compensation_1c)/3)
...
b3 <- b2 %>%
        pivot_wider(id_cols = c('Subj','Age','Sex'),
                    names_from = TimepointNr,
                    names_sep = '_',
                    values_from = c('Pathology_3gt1','Pathology_2gt1','Pathology_Mean',
                                    'Compensation_3gt1','Compensation_2gt1','Compensation_Mean',
                                    'ClinScore_imp2')) %>%
        mutate(Pathology_3gt1_Delta = Pathology_3gt1_T1 - Pathology_3gt1_T0,
               Pathology_2gt1_Delta = Pathology_2gt1_T1 - Pathology_2gt1_T0,
               Pathology_Mean_Delta = Pathology_Mean_T1 - Pathology_Mean_T0,
               Compensation_3gt1_Delta = Compensation_3gt1_T1 - Compensation_3gt1_T0,
               Compensation_2gt1_Delta = Compensation_2gt1_T1 - Compensation_2gt1_T0,
               Compensation_Mean_Delta = Compensation_Mean_T1 - Compensation_Mean_T0,
               ClinScore_imp2_Delta = ClinScore_imp2_T1 - ClinScore_imp2_T0) %>%
        na.omit()

# Separate variables
b4 <- b3 %>%
        select(Subj,Age,Sex,ClinScore_imp2_Delta,ClinScore_imp2_T0, starts_with('Pathology'), starts_with('Compensation')) %>%
        pivot_longer(cols = c(starts_with('Pathology'), starts_with('Compensation')),
                     names_to = c('Network','Composite','Timepoint'),
                     names_pattern = '(.*)_(.*)_(.*)',
                     values_to = 'Value') %>%
        pivot_wider(id_cols = c('Subj','Age','Sex','ClinScore_imp2_Delta','ClinScore_imp2_T0','Network','Composite'),
...
b5 <- b4 %>%
        filter(Composite=='Mean')

m <- lm(Beta_Delta ~ ClinScore_imp2_Delta*Network + Beta_T0 + ClinScore_imp2_T0, data = b5)
summary(m)
ggemmeans(m, terms = c('ClinScore_imp2_Delta','Network')) %>% plot()

betas2 <- read_csv('P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI/ROI_BetaExtraction/con_0010_severity-poly1_23-Jan-2023_dataTable.txt')
# Define measures of interest (i.e. average betas across pathology and compensation networks)
b1 <- betas2 %>%
        select(Subj, TimepointNr, ClinScore_imp2, Age, Sex, starts_with('HCgtPD_Mean'), starts_with('Neg_')) %>%
        select(!contains('PCA')) %>%
        rowwise() %>%
        mutate(Pathology = mean(c_across(contains('HCgtPD_Mean'))),
               Compensation = mean(c_across(contains('Neg_')))) %>%
        select(Subj, TimepointNr, ClinScore_imp2, Age, Sex, Pathology, Compensation) 
...
b2 <- b1 %>%
        pivot_wider(id_cols = c('Subj','Age','Sex'),
                    names_from = TimepointNr,
                    names_sep = '_',
                    values_from = c('Pathology', 'Compensation', 'ClinScore_imp2')) %>%
        mutate(Pathology_Delta = Pathology_T1 - Pathology_T0,
               Compensation_Delta = Compensation_T1 - Compensation_T0,
               ClinScore_imp2_Delta = ClinScore_imp2_T1 - ClinScore_imp2_T0) %>%
        na.omit()
...
b3 <- b2 %>%
        select(Subj,Age,Sex,ClinScore_imp2_Delta,ClinScore_imp2_T0, starts_with('Pathology'), starts_with('Compensation')) %>%
        pivot_longer(cols = c(starts_with('Pathology'), starts_with('Compensation')),
                     names_to = c('Network','Timepoint'),
                     names_pattern = '(.*)_(.*)',
                     values_to = 'Value') %>%
        pivot_wider(id_cols = c('Subj','Age','Sex','ClinScore_imp2_Delta','ClinScore_imp2_T0','Network'),
                    names_from = Timepoint,
                    names_prefix = 'Beta_',
                    values_from = Value) %>%
        mutate(Age=Age-mean(Age),
               Sex=factor(Sex),
               Network=factor(Network),
               ClinScore_imp2_Delta = ClinScore_imp2_Delta - mean(ClinScore_imp2_Delta),
               ClinScore_imp2_T0 = ClinScore_imp2_T0 - mean(ClinScore_imp2_T0),
               Beta_T0 = Beta_T0 - mean(Beta_T0)) %>%
        arrange(Subj)

m <- lm(Beta_Delta ~ ClinScore_imp2_Delta*Network + Beta_T0 + ClinScore_imp2_T0, data = b3)
summary(m)
ggemmeans(m, terms = c('ClinScore_imp2_Delta','Network')) %>% plot()



betas3 <- read_csv('P:/3024006.02/Analyses/DurAvg_ReAROMA_PMOD_TimeDer_Trem/Group/Longitudinal/AFNI/ROI/3dttest++_severity/stats/con_0010_dataTable_24-Jan-2023.txt')

betas3 %>%
        ggplot(aes(y=ClinScore_delta,x=con_0010_severity_T_Delta_cid1)) + 
        geom_point() +
...
dat.int <- dat %>%
    select(Subj, TimepointNr, trial_type, ClinScore, Age, Sex, MeanFD, Z_SeverityType2gt1_cid1, Z_SeverityType3gt1_cid1)
...
interrogate_cluster(dat.int, 'Z_SeverityType3gt1_cid1', 'H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri')
...
progression <- dat.int.collapsed %>% pivot_wider(id_cols = c('Subj'),
                                                 names_from = TimepointNr,
                                                 values_from = c(ClinScore,
                                                                 Z_SeverityType2gt1_cid1_2gt1,
                                                                 Z_SeverityType3gt1_cid1_3gt1)) %>%
    mutate(ClinScore.BA=ClinScore_T0,
           ClinScore.delta=ClinScore_T1-ClinScore_T0,
           Severity2gt1.delta = Z_SeverityType2gt1_cid1_2gt1_T1 - Z_SeverityType2gt1_cid1_2gt1_T0,
           Severity3gt1.delta = Z_SeverityType3gt1_cid1_3gt1_T1 - Z_SeverityType3gt1_cid1_3gt1_T0) %>%
    select(Subj, ClinScore.BA, ClinScore.delta, Severity2gt1.delta, Severity3gt1.delta)
...
g <- ggplot(dat.int, aes(y=Z_SeverityType2gt1_cid1,x=ClinScore)) + 
    ...
    facet_grid(~trial_type)+
    theme_bw() + 
    ylab('Beta (two > one)') + xlab('Bradykinesia') +
...
g <- ggplot(dat.int.collapsed, aes(y=Z_SeverityType2gt1_cid1_2gt1,x=ClinScore)) + 
    ...
    geom_smooth(color='darkred',method='lm') +
    theme_bw() +
    labs(color='Progression') + ylab('Beta (two > one)') + xlab('Bradykinesia') +
    ggtitle('Change in selection-related activity predicts progression')
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-2gt1-collapsed.png',
          g, dpi=300, device='png', base_width = 5)
# Collapsed across condition and time
m.2gt1 <- lmer(Z_SeverityType2gt1_cid1 ~ ClinScore*trial_type + Age + Sex + (ClinScore|Subj), 
               data = dat.int,
               control = lmerControl(optimizer = 'bobyqa'))
tab_model(m.2gt1)
s <- summary(m.2gt1)
g <- dat.int.collapsed %>%
    filter(TimepointNr=='T0') %>%
    ggplot(aes(y=Severity2gt1.delta, x=ClinScore.delta)) +
    geom_point(alpha=0.25,size=0.6) +
    geom_smooth(color='darkred',method='lm') +
    theme_bw() +
    labs(color='Progression') + ylab('Delta (two > one, fu > ba)') + xlab('Bradykinesia (fu > ba)') +
    ggtitle('Change in selection-related activity predicts progression') + 
    geom_text(x=0,y=2.5, label = paste('β = ', round(s$coefficients[7,1],digits=3),
                                       ', SE = ', round(s$coefficients[7,2],digits=3),
                                       ', P = ', round(s$coefficients[7,5],digits=10),
                                       sep=''))
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_DeltaSev-x-Delta2gt1-collapsed.png',
          g, dpi=300, device='png', base_width = 5)
# Predicted trends
e <- ggemmeans(m.2gt1, terms = c('ClinScore','trial_type'))
g <- plot(e) + 
    ggtitle('Change in selection-related activity predicts progression') + 
    labs(y='Beta',x='Bradykinesia')
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-2gt1-pred.png',
          g, dpi=300, device='png', base_width = 5)
# Predicted trends collapsed across condition and time
tmp <- dat.int.collapsed %>%
    filter(TimepointNr == 'T0') %>%
    select(Subj, ClinScore.BA, Age, Sex, ClinScore.delta, Severity2gt1.delta, Severity3gt1.delta)
m.2gt1.delta <- lm(Severity2gt1.delta ~ ClinScore.delta + Age + Sex, data = tmp, weights = ClinScore.BA)
tab_model(m.2gt1.delta)
e <- ggemmeans(m.2gt1.delta, terms = 'ClinScore.delta [all]')
g <- plot(e, residuals = TRUE, residuals.line = TRUE) + 
    ggtitle('Change in selection-related activity predicts progression') + 
    labs(y='Beta (two > one, fu > ba)',x='Bradykinesia (fu > ba)')
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-2gt1-pred-collapsed.png',
          g, dpi=300, device='png', base_width = 5)
# All effects
ae <- allEffects(m.2gt1)
g <- plot(ae)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-2gt1-alleffs.png',
          g, dpi=300, device='png', base_width = 5)
# SPL, 3>1
# Full data
g <- ggplot(dat.int, aes(y=Z_SeverityType3gt1_cid1,x=ClinScore)) + 
    ...
    facet_grid(~trial_type)+
    theme_bw() + 
    ylab('Beta (three - one)') + xlab('Bradykinesia') +
    ggtitle('Change in selection-related activity predicts progression')
print(g)
...
g <- ggplot(dat.int.collapsed, aes(y=Z_SeverityType3gt1_cid1_3gt1,x=ClinScore)) + 
    ...
    geom_smooth(color='darkred',method='lm') +
    theme_bw() +
    labs(color='Progression') + ylab('Beta (three - one)') + xlab('Bradykinesia') +
    ggtitle('Change in selection-related activity predicts progression')
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-3gt1-collapsed.png',
          g, dpi=300, device='png', base_width = 5)
# Collapsed across condition and time
m.3gt1 <- lmer(Z_SeverityType3gt1_cid1 ~ ClinScore*trial_type + Age + Sex + (ClinScore|Subj), 
               data = dat.int,
               control = lmerControl(optimizer = 'bobyqa'))
tab_model(m.3gt1)
s <- summary(m.3gt1)
g <- dat.int.collapsed %>%
    filter(TimepointNr=='T0') %>%
    ggplot(aes(y=Severity3gt1.delta, x=ClinScore.delta)) +
    geom_point(alpha=0.25,size=0.6) +
    geom_smooth(color='darkred',method='lm') +
    theme_bw() +
    labs(color='Progression') + ylab('Delta (three > one, fu > ba)') + xlab('Bradykinesia (fu > ba)') +
    ggtitle('Change in selection-related activity predicts progression') + 
    geom_text(x=0,y=2.5, label = paste('β = ', round(s$coefficients[8,1],digits=3),
                                       ', SE = ', round(s$coefficients[8,2],digits=3),
                                       ', P = ', round(s$coefficients[8,5],digits=10),
                                       sep=''))
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_DeltaSev-x-Delta3gt1-collapsed.png',
          g, dpi=300, device='png', base_width = 5)
# Predicted trends
e <- ggemmeans(m.3gt1, terms = c('ClinScore','trial_type'))
g <- plot(e) + 
    ggtitle('Change in selection-related activity predicts progression') + 
    labs(y='Beta',x='Bradykinesia')
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-3gt1-pred.png',
          g, dpi=300, device='png', base_width = 5)
# Predicted trends collapsed across condition and time
tmp <- dat.int.collapsed %>%
    filter(TimepointNr == 'T0') %>%
    select(Subj, ClinScore.BA, Age, Sex, ClinScore.delta, Severity2gt1.delta, Severity3gt1.delta)
m.3gt1.delta <- lm(Severity3gt1.delta ~ ClinScore.delta + Age + Sex, data = tmp, weights = ClinScore.BA)
tab_model(m.3gt1.delta)
e <- ggemmeans(m.3gt1.delta, terms = 'ClinScore.delta [all]')
g <- plot(e, residuals = TRUE, residuals.line = TRUE) + 
    ggtitle('Change in selection-related activity predicts progression') + 
    labs(y='Beta (three > one, fu > ba)',x='Bradykinesia (fu > ba)')
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-3gt1-pred-collapsed.png',
          g, dpi=300, device='png', base_width = 5)
# All effects
ae <- allEffects(m.3gt1)
g <- plot(ae)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMER_severity_Sev-x-3gt1-alleffs.png',
          g, dpi=300, device='png', base_width = 5)
...
dat.int %>%
    bind_cols(., yhat=predict(m.2gt1)) %>%
    ggplot(aes(x=ClinScore,y=yhat,color=trial_type)) + 
...
estimates1 <- dat.int %>%
    bind_cols(., pred = predict(m.2gt1)) %>%
    
    
    
    
    dat.int.m %>%
    ggplot(aes(x=TimepointNr, y=yhat2gt1)) + 
    geom_point() + 
    geom_boxplot()

m.3gt1 <- lmer(Z_SeverityType3gt1_cid1 ~ ClinScore*trial_type + (ClinScore|Subj), 
               data = dat.int,
               control = lmerControl(optimizer = 'bobyqa'))
tab_model(m.3gt1)
...
t <- dat.int.collapsed %>%
    filter(TimepointNr == 'T0') %>%
    select(Subj, ClinScore.BA, Age, Sex, ClinScore.delta, Severity2gt1.delta, Severity3gt1.delta)
testm <- lm(Severity2gt1.delta ~ ClinScore.delta + Age + Sex, data = t, weights = ClinScore.BA)
e <- ggemmeans(testm, terms = 'ClinScore.delta')
plot(e)
ae <- allEffects(testm)
plot(ae)
...
m.3gt1 <- lmer(Z_SeverityType3gt1_cid1 ~ ClinScore*trial_type + Age + Sex + (ClinScore|Subj), 
               ...
               control = lmerControl(optimizer = 'bobyqa'))
summary(m.3gt1)
allEffects(m.3gt1) %>% plot()
ggemmeans(m.3gt1, terms = c('ClinScore','trial_type')) %>% plot()
...
m <- lmer(Z_SeverityType2gt1_cid1 ~ ClinScore*trial_type + scale(Age) + factor(Sex) + (ClinScore|Subj), 
          data = dat.int,
          control = lmerControl(optimizer = 'bobyqa'))
...
plot_resid <- ggplot(data=estimates1,
                     aes(x = ClinScore, y = resid, group = Subj)) +
    geom_line() +
    theme_bw() +
    facet_grid(~trial_type)
