...
library(car)
library(extrafont)
library(ggpubr);library(ggforce);library(gghalves);library(ggdist);library(ggeffects); library(GGally); library(sjPlot); library(cowplot); library(viridis); library(lattice); library(see); library(kableExtra)
...
source('H:/sysneu/marjoh/scripts/Personalized-Parkinson-Project-Motor/R/functions/plot_rainclouds.R')
source('H:/sysneu/marjoh/scripts/Personalized-Parkinson-Project-Motor/R/functions/raincloudplot_1Factor.R')
source('H:/sysneu/marjoh/scripts/Personalized-Parkinson-Project-Motor/R/functions/raincloudplot_2Factor.R')
...
vars.prog <- c('pseudonym', 'TimepointNr', 'Age', 'Gender', 'YearsSinceDiag',
               'ParticipantType', 'Up3OfBradySum', 'Up3OnBradySum', 'Up3BradySum','z_MoCA__total')
dfClin.prog <- dfClin %>%
...
dfClin.prog.imp <- dfClin.prog %>%
        select(pseudonym, Up3OfBradySum, Up3OnBradySum, z_MoCA__total, TimepointNr, Age, Gender, YearsSinceDiag) %>%
...
dfClin.prog.imp <- imputed %>%
        ...
        select(pseudonym, TimepointNr, Up3BradySum, z_MoCA__total) %>%
...
dfClin.prog <- dfClin.prog %>%
    pivot_wider(id_cols = c('pseudonym','ParticipantType'),
                names_from = TimepointNr,
                names_prefix = 'T',
                values_from = c(Up3OfBradySum,Up3OnBradySum,Up3BradySum,Up3BradySum.imp,z_MoCA__total,z_MoCA__total.imp))
...
dfFMRI <- dfFMRI %>%
    mutate(Age_timevar=if_else(TimepointNr=='2',Age+YearsToFollowUp,Age),
           ...
           z_MoCA__total = z_MoCA__total_T0,
           z_MoCA__total = if_else(TimepointNr=='2',z_MoCA__total_T0+z_MoCA__total.imp_RawChange,z_MoCA__total))
dfPrac <- dfPrac %>%
    mutate(Age_timevar=if_else(TimepointNr=='2',Age+YearsToFollowUp,Age),
           Age_Dmean = Age - mean(dfFMRI$Age[dfFMRI$TimepointNr==0],na.rm=TRUE),
           YearsSinceDiag_timevar=if_else(TimepointNr=='2',YearsSinceDiag+YearsToFollowUp,YearsSinceDiag),
...
# Responding hand
dfFMRI %>% group_by(pseudonym,TimepointNr) %>% summarise(ParticipantType=first(ParticipantType),RespondingHand=first(RespondingHand)) %>% ungroup() %>% select(-pseudonym) %>% group_by(TimepointNr) %>% table()
...
### CONTINUE REVISIONS HERE >>> *Group comparison

```{r rts-fMRI, echo=FALSE, warning=TRUE, message=TRUE}

f <- 'log(response_time) ~ 1 + ParticipantType*TimepointNr*trial_type + block + Age + Gender + NpsEducYears + RespHandIsDominant + (1+TimepointNr|pseudonym)'
...
g <- tmp.collapsed %>%
        ...
        ylab('Response time (s)') + xlab('') + 
        guides(color = guide_legend(title='Choice')) +
        ylim(0.45, 1.5) + 
        facet_wrap(~TimepointNr)
y <- 1.4
d = 0.1
segmap <- data.frame(
        x =    c(1.00, 0.80,  1.80),
...
g <- g + 
        ...
        geom_text(data=sigmap, 
                  ...
                  size = sigsize)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/RT_lmer.png',
          g, dpi=300, device='png', base_width = 5)
...
#         ungroup() %>%
#         rmcorr(pseudonym,Up3BradySum,response_time,.)
# print(rmc)
# plot(rmc)
...
f <- 'error_rate ~ 1 + ParticipantType*TimepointNr*trial_type + block + Age + Gender + NpsEducYears + RespHandIsDominant + (1+TimepointNr|pseudonym)'
...
g <- g + 
        ...
        geom_text(data=sigmap, 
                  ...
                  size = sigsize)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/ERR_lmer.png',
          g, dpi=300, device='png', base_width = 5)
...
m <- lmer(Up3BradySum ~ TimepointNr*Medication + Age + Gender + YearsSinceDiag + (1+TimepointNr|pseudonym), data = tmp)
tab_model(m, digits=5,show.se = T)
Anova(m, type=3) %>% print()
...
g <- tmp %>%
        ...
        scale_color_viridis_d(option='mako', begin = .1, end = .6,
                              direction = -1) + 
        ggtitle('Bradykinesia progression') +
...
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/ClinProg_lmer_Brady.png',
          g, dpi=300, device='png', base_width = 5)

##### Check effect of time on medication
tmp <- dfClin %>%
        filter(TimepointNr != 1) %>%
        select(pseudonym,TimepointNr,Up3OfBradySum,Up3OnBradySum,LEDD,Age,Gender,YearsSinceDiag) %>%
        pivot_longer(cols = c('Up3OfBradySum','Up3OnBradySum'),
                     names_to = 'Medication',
                     values_to = 'Up3BradySum') %>%
        mutate(Medication=factor(Medication,
...
m.LEDD <- tmp %>%
        ...
        lmer(LEDD ~ TimepointNr + Age + Gender + YearsSinceDiag + (1|pseudonym), data = .)
s <- summary(m.LEDD)
print(s)
...
rmc <- tmp %>%
        filter(Medication=='On') %>%
        rmcorr(pseudonym, LEDD, Up3BradySum, .)
print(rmc)
plot(rmc)
...
tmp <- dfClin %>%
        filter(TimepointNr != 1) %>%
        select(pseudonym,TimepointNr,z_MoCA__total,Age,Gender,YearsSinceDiag) %>%
        mutate(TimepointNr=factor(TimepointNr),
               Age=Age-mean(Age,na.rm=T),
               YearSinceDiag=YearsSinceDiag-mean(YearsSinceDiag,na.rm=T)) %>%
        na.omit()
...
m <- lmer(z_MoCA__total ~ TimepointNr + Age + Gender + YearsSinceDiag + (1|pseudonym), data = tmp)
tab_model(m, digits=5,show.se = T)
Anova(m, type=3) %>% print()
...
g <- tmp %>%
        ...
        guides(color = guide_legend(title='Year')) +
        annotate(geom='text',label='N=351',x=1,y=-3.5) + 
...
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/ClinProg_lmer_MoCA.png',
          g, dpi=300, device='png', base_width = 5)
...
```{r fsl-rand-unpaired_motor-tstat1, echo=FALSE, warning=TRUE, message=TRUE, eval=TRUE}
...
```{r fsl-rand-unpaired_motor-tstat2, echo=FALSE, warning=TRUE, message=TRUE, eval=TRUE}
...
```{r fsl-rand-unpaired_as-tstat2, echo=FALSE, warning=TRUE, message=TRUE, eval=TRUE}
...
```{r fsl-rand-clincorr_as-tstat2, echo=FALSE, warning=TRUE, message=TRUE, eval=TRUE}
...
```{r pd-vs-hc, echo=FALSE, warning=TRUE, message=TRUE, eval=T}

# Group x Time
dat <- read_csv('P:/3024006.02/Analyses/motor_task/Group/Longitudinal/AFNI/ROI/Masked_full/3dLME_disease/stats/con_combined_Group2_x_TimepointNr2_x_Type3_z_Group_by_Time_dataTable_21-Feb-2024.txt') %>%
        left_join(., read_csv('P:/3024006.02/Analyses/motor_task/Group/Longitudinal/AFNI/ROI/Masked_full/3dLME_disease/stats/con_combined_Group2_x_TimepointNr2_x_Type3_Z_Group_dataTable_21-Feb-2024.txt')) %>%
        ...
        summarise(con_combined__z_Group_by_Time_cid1 = mean(con_combined__z_Group_by_Time_cid1),
...
# Group x Time
g <- dat %>%
        ggplot(aes(x=Group,y=con_combined__z_Group_by_Time_cid1,color=TimepointNr)) + 
        geom_jitter(alpha=0.2, position = position_jitterdodge(jitter.width = 0.2)) +
        geom_boxplot(outlier.shape = NA, width=0.5, position = position_dodge(width=0.8)) + 
        stat_summary(fun.data = 'mean_cl_boot', geom = 'point', color='darkred', size=1.5, 
                     mapping = aes(group=TimepointNr), position = position_dodge(width=0.8)) +
        ...
        scale_color_viridis_d(option='mako', begin = .1, end = .6,
                              direction = -1) + 
        ggtitle('Longitudinal increase in cerebellar activity') +
        ylab('Cerebellar lobule V activity (beta)') + xlab('') + 
        guides(color = guide_legend(title='Year')) +
        annotate(geom='text',label='N=60',x=0.8,y=-4) + 
        annotate(geom='text',label='N=52',x=1.2,y=-4) +
        annotate(geom='text',label='N=332',x=1.8,y=-4) + 
        annotate(geom='text',label='N=276',x=2.2,y=-4) +
        ylim(-4.1, 12)
y <- 11
d = 0.75
segmap <- data.frame(
...
g <- g + 
        ...
        geom_text(data=sigmap, 
                  ...
                  size = sigsize)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMEr_Group_by_Time_cb5.png',
          g, dpi=300, device='png', base_width = 5)
...
g <- g + 
        ...
        geom_text(data=sigmap, 
                  ...
                  size = sigsize)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMEr_Group_putamen.png',
          g, dpi=300, device='png', base_width = 5)
...
#         save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dLMEr_disease_TimeXGroup.png', ., dpi=300)
# 
...
```{r brain-clin-fsl, echo=FALSE, warning=TRUE, message=TRUE, eval=FALSE}
...
dat_delta <- read_csv('P:/3024006.02/Analyses/motor_task/Group/Longitudinal/FSL/stats/con_0007/vals/rand_delta_clincorr_all_vxlEV_tfce_corrp_tstat2_stats_stats_avg_agg.txt',
                      col_names = c('Subj','cid1_delta','cid2_delta','cid3_delta','cid4_delta')) %>%
...
dat_ba <- read_csv('P:/3024006.02/Analyses/motor_task/Group/Longitudinal/FSL/stats/con_0007/vals/rand_delta_clincorr_all_vxlEV_tfce_corrp_tstat2_stats_BASELINE_stats_avg_agg.txt',
                   col_names = c('Subj','cid1_ba','cid2_ba','cid3_ba','cid4_ba')) %>%
        select(-Subj)
...
dat <- bind_cols(dat_delta, dat_ba, dat_covs) %>%
        mutate(Gender = factor(Gender),
               RespHandIsDominant = factor(RespHandIsDominant))

m <- dat %>%
        lm(cid4_delta ~ Up3OnBradySum_Delta + Up3OnBradySum_T0 + cid4_ba + Age + Gender + NpsEducYears + RespHandIsDominant, data = .)

tab_model(m,title='Brain-Clinical Associations',digits=5,show.se = T)
ols_vif_tol(m) %>% print()
Anova(m,type=3) %>% print()
LmPlots(m)
pred <- ggpredict(m, terms='Up3OnBradySum_Delta')
g <- plot(pred, add.data = TRUE) + theme_bw() + ggtitle('') +
        ylab('Change in activity (Multiple > Single, z)') + xlab('Change in bradykinesia (ON)') +
        annotate('text',y=5,x=-25,label='N=265, β=-0.050, SE=0.011, P<0.001 (FWEc-corrected)', hjust=0) +
        ylim(-4,6) + 
        geom_hline(yintercept = 0, color = 'darkgrey', lty = 2) + geom_vline(xintercept = 0, color = 'darkgrey', lty = 2)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/randomise_severity_23gt1.png',
                  g, dpi=300, device='png', base_width = 5)
```
...
g <- plot(pred, add.data = TRUE) + theme_bw() + ggtitle('') +
        ...
        ylim(-12,15)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dttest++_severity_Mean.png',
                  g, dpi=300, device='png', base_width = 5)
...
g <- plot(pred, add.data = TRUE) + theme_bw() + ggtitle('Longitudinal alterations in parieto-premotor activity correlate with clinical progression') +
        ...
        ylim(-3.5,5.2)
print(g)
save_plot('H:/sysneu/marjoh/Visualization/R_LongitudinalComparisonFmri/3dttest++_severity_23gt1.png',
                  g, dpi=300, device='png', base_width = 5)
